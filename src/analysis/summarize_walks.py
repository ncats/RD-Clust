...
def main():
    ...
    node_strings = [str(i) for i in disease_ontograph.nodes if 'GARD' not in i]
    for d in diseases_inwalks[0:10]:
        ...
        dis_clust_annot = [i for i in dis_w if i in node_strings]
        annot_counts = Counter(dis_clust_annot)
        cluster_walk_annotations.extend([[str(clust),d,disease_ontograph.nodes[a[0]].get('label'),a[0],str(a[1])] for a in annot_counts.items()])

    print(cluster_walk_annotations)
