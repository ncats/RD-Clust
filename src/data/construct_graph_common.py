...
def process_pharos_data(pharos_ligand_data,pharos_disease_data,small_molecule_set,gene2go,gene_ontology):
    """
    Construct graph from pharos data
    """
    pharos_graph = nx.MultiGraph()

    ligand_uniprot = set([i.get('UniProt') for i in pharos_ligand_data])
    disease_uniprot = set([i.get('UniProt') for i in pharos_disease_data])
    pharos_uniprot = ligand_uniprot.union(disease_uniprot)
    ...
    print("Adding pharos ligand data\n")
    ec = 0
    for row in pharos_ligand_data:
        edge = []
    ...
    print("Adding pharos disease data\n")
    ec = 0
    for row in pharos_disease_data:
        edge = []
        uniprot = row.get('UniProt')
        ...
        if uniprot in uniprot2entrez:
            for gene in uniprot2entrez[uniprot]:
                ...
                edge.append(int(gene))
                gene_annotation = gene2go.get(int(gene))
                #TODO:make this a function, I repeat it way too much
                if gene_annotation is not None:
                    go_leafs = get_leafs(gene_ontology,[i['GO_ID'] for i in gene_annotation])
                ...
                if mondo:
                    pharos_graph.add_node(mondo,label='disease')
                    edge.append(mondo)
                else:
                    continue
                ...
                if len(edge) == 2:
                    ec += 1
                    pharos_graph.add_edge(edge[0],edge[1],key="associated_with")
...
def add_mondo(disease_subgraph,mondo_phen_df,phenotype_ontology):

    for dis in set(mondo_phen_df['Mondo ID']):
        print(dis)
        phens = mondo_phen_df['HPO_ID'][mondo_phen_df['Mondo ID'] == dis]
        
        disease_subgraph.add_node(dis,label='disease')
        
        hpo_phens = [i for i in phens if 'HP:' in i]
        nonhpo_phens = [i for i in phens if 'HP:' not in i]
        
        hpo_leafs = get_leafs(phenotype_ontology,hpo_phens)
        print(hpo_leafs)
        print(nonhpo_phens)
        for hpo in hpo_leafs:
            disease_subgraph.add_node(hpo,label='HPO')
            disease_subgraph.add_edge(dis,hpo,key="presents")

        for pheno in nonhpo_phens:
            disease_subgraph.add_node(pheno,label='HPO') #for now leave as HPO to be clear
            disease_subgraph.add_edge(dis,pheno,key="presents")
...
def main():
    ...
    #MONDO - GARD mappings:
    mondo2gard={}
    with open(project_dir / 'data/raw/gard2mondo.csv','r',encoding='utf-8-sig') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            mondo2gard[row.get('MONDO_ID')] = row.get('GARD_ID')
    ...
    mondo_phen_df = pd.read_csv(project_dir / 'data/raw/mondo_hpo.csv')

    gard_phen_df.drop(gard_phen_df[gard_phen_df['HPO_ID'].isin(hp_remove)].index, inplace = True)
    mondo_phen_df.drop(mondo_phen_df[mondo_phen_df['HPO_ID'].isin(hp_remove)].index, inplace = True)
    for index, row in mondo_phen_df.iterrows():
        if row['Mondo ID'] in mondo2gard:
            mondo_phen_df['Mondo ID'][index] = mondo2gard[row['Mondo ID']]
    ...
    with open(project_dir / 'data/raw/pharos_all_target_ligands.csv','r') as pharos_ligand_file:
        ...
        for row in reader:
            pharos_ligand_data.append(row)
    
    pharos_disease_data = []
    with open(project_dir / 'data/raw/pharos_all_target_diseases.csv','r') as pharos_disease_file:
        reader = csv.DictReader(pharos_disease_file)
        for row in reader:
            if row.get('Mondo ID') in mondo2gard:
                row['Mondo ID'] = mondo2gard[row.get('Mondo ID')]
            pharos_disease_data.append(row)
    
    disease_subgraph = construct_disease_subgraph(gard_gene_df, gard_phen_df,gene2go,gene_ontology,phenotype_ontology)
    disease_subgraph = add_mondo(disease_subgraph,mondo_phen_df,phenotype_ontology)
    ...
    pharos_graph = process_pharos_data(pharos_ligand_data,pharos_disease_data,small_molecule_set,gene2go,gene_ontology)
